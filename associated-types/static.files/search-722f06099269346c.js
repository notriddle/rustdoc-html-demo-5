...
/**
    ...
    function execQuery(parsedQuery, searchWords, filterCrates, currentCrate) {
        ...
        function checkType(row, elem, whereClause) {
            if (row.id === null) {
                for (const constraints of row.bindings.values()) {
                    if (checkIfInList(constraints, elem, whereClause)) {
                        return true;
                    }
                }
            ...
            if ((matchesExact || matchesArrayOrSlice) &&
                ...
                if (elem.generics.length > 0 || elem.bindings.size > 0) {
            ...
            if (checkIfInList(row.generics, elem, whereClause)) {
                return true;
            }
            for (const constraints of row.bindings.values()) {
                if (checkIfInList(constraints, elem, whereClause)) {
